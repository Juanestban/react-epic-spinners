...
const rotation = (number) => `rotate(${number}deg)`

const scale = (number) => `scale(${number})`
...
export const FulfillBounCircleContainer = styled.div`
  ...
  height: ${getSize};
  position: relative;
  animation: fulfilling-bounding-container-animation infinite ${getDuration}
    ease;
  ...
  & .orbit {
    ...
    animation: fulfilling-bounding-orbit-animation infinite ${getDuration} ease;
  }
  ...
  & .circle {
    ...
    color: ${getColor};
    display: block;
    animation: fulfilling-bounding-circle-animation infinite ${getDuration} ease;
    border-radius: 50%;
    position: relative;
